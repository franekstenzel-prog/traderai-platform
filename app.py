...
# -----------------------------
def analyze_with_openai_pro(image_bytes, pair, timeframe, mode, capital, risk_fraction, spread_bps=None, fee_bps=None, slippage_bps=None, aggressiveness=None, **_ignored):
    ...
    aggr = (aggressiveness or "").strip().lower()
    if aggr not in ("strict", "balanced", "aggressive"):
        # Default to balanced (more signals than strict, still sane).
        aggr = "balanced"

    # Aggressiveness tuning (trade frequency vs selectivity)
    if aggr == "strict":
        near_btc_eth_pct = 0.15
        near_alt_pct = 0.25
        require_candle_confirm = True
        strict_ambiguity_no_trade = True
        min_rr_scalp = 1.2
        min_rr_swing = 1.5
    elif aggr == "aggressive":
        near_btc_eth_pct = 0.50
        near_alt_pct = 0.80
        require_candle_confirm = False
        strict_ambiguity_no_trade = False
        min_rr_scalp = 1.0
        min_rr_swing = 1.1
    else:  # balanced
        near_btc_eth_pct = 0.30
        near_alt_pct = 0.50
        require_candle_confirm = False
        strict_ambiguity_no_trade = False
        min_rr_scalp = 1.1
    ...
    confirm_rule = (
        ...
        else "Candlestick confirmation is helpful but NOT mandatory. If missing, lower confidence and list it in issues."
    )
    ambiguity_rule = (
        "If anything is ambiguous -> NO_TRADE. This is mandatory."
        if strict_ambiguity_no_trade
        else "If something is ambiguous, still propose the best plan but cap confidence <=55 and list issues. Use NO_TRADE only if chart unreadable or truly no edge."
    )
    ...
    instruction = f"""
You are a professional discretionary trading assistant.

GOAL: produce a trade plan that is LOGICAL and ACTIONABLE (not over-cautious).
Aggressiveness profile: {aggr.upper()}.
...
2) Identify KEY SUPPORT & RESISTANCE levels/zones (horizontal zones + trendlines + channels).
   Treat levels as ZONES (not a single precise line). Prefer zones with 2–3+ reactions.
3) Look for a setup when CURRENT PRICE is close to a key zone:
   - BTC/ETH vs USDT/USDC: within ~{near_btc_eth_pct}% of a zone
   - other crypto pairs: within ~{near_alt_pct}% of a zone
   Use reasonable zone width based on visible volatility; allow limit entries inside the zone.
   If price is clearly mid-range and there is no obvious edge (no major level / boundary) -> signal=NO_TRADE.
4) Entry logic:
   - LONG: near SUPPORT (or support retest after a bounce).
           SL must be BELOW the support zone and below nearest swing low/liquidity.
           TP targets = nearest resistance zone(s).
   - SHORT: near RESISTANCE (or resistance retest after rejection).
            SL must be ABOVE the resistance zone and above nearest swing high/liquidity.
            TP targets = nearest support zone(s).
5) Confirmation logic (secondary):
   - {confirm_rule}
   Use candlestick patterns only as *extra* confirmation (engulfing, pin bar, doji rejection, inside-bar breakout).
6) Uncertainty rule:
   - {ambiguity_rule}

OUTPUT:
- Always return STRICT JSON ONLY (no markdown, no commentary), matching the provided JSON schema.
- If signal=NO_TRADE: entry=null, stop_loss=null, take_profit=[].
...
"""
    ...
    if sig in ("LONG", "SHORT") and entry_n is not None and sl_n is not None:
        ...
        if tp0_n is not None:
            ...
            min_rr = min_rr_scalp if mode_norm == "scalp" else min_rr_swing
...
def analyze():
    ...
    aggressiveness = (request.form.get("aggressiveness") or row_get(user, "default_aggressiveness") or "balanced").strip().lower()
    ...
    db = get_db()
    db.execute(
        "UPDATE users SET default_pair = ?, default_timeframe = ?, default_capital = ?, default_risk_fraction = ?, default_mode = ?, default_aggressiveness = ? WHERE id = ?",
        (pair, timeframe, capital, risk_fraction, mode, aggressiveness, user["id"]),
    )
    db.commit()
    ...
    try:
        result = analyze_with_openai_pro(
            ...
            fee_bps=fee_bps,
            slippage_bps=slippage_bps,mode=mode, aggressiveness=aggressiveness,
